...
implements INetworkItemEventListener {
    ...
    public List<String> getHudInfo(ItemStack stack) {
        ...
        NBTTagCompound nbtData = StackUtil.getOrCreateNbtData(stack);
        String mode = Localization.translate(ItemToolMiningLaser.getModeString(nbtData.getInteger("laserSetting")));
        ...
        if (!(IC2.keyboard.isModeSwitchKeyDown(player) || nbtData.getInteger("laserSetting") != 3 && nbtData.getInteger("laserSetting") != 7)) {
            ...
            if (Math.abs(angle) < 1.0 / Math.sqrt(2.0)) {
                if (ElectricItem.manager.use(stack, 3000.0, (EntityLivingBase)player)) {
                    ...
                    if (nbtData.getInteger("laserSetting") == 3 && this.shootLaser(stack, world, start, dir, (EntityLivingBase)player, Float.POSITIVE_INFINITY, 5.0f, Integer.MAX_VALUE, false, false)) {
                        IC2.network.get(true).initiateItemEvent(player, stack, 3, true);
                    }
                    if (nbtData.getInteger("laserSetting") == 7 && this.shootLaser(stack, world, start, dir, (EntityLivingBase)player, Float.POSITIVE_INFINITY, 5.0f, Integer.MAX_VALUE, false, false)) {
